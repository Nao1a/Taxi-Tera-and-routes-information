      ...
      {tab==='submissions' && (<div className="space-y-3">
        {items.map(it => (
          <div key={it._id} className="p-4 border rounded bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-700">
            ...
            {it.type === 'driver_verification' && renderDriverVerification(it)}
            {it.type === 'route_application' && renderRouteApplication(it)}
            {it.type !== 'driver_verification' && it.type !== 'route_application' && (
              <>
...
// Render driver verification submission
function renderDriverVerification(it) {
  const p = it?.payload || {};
  return (
    <div className="mt-4 space-y-4">
      <div className="grid md:grid-cols-2 gap-4">
        <div>
          <div className="font-semibold mb-2">License Information</div>
          <div className="text-sm text-gray-600 dark:text-gray-400 mb-2">License Number: {p.licenseText || 'N/A'}</div>
          {p.licensePhoto && (
            <div className="mt-2">
              <div className="text-xs text-gray-500 mb-1">License Photo:</div>
              <img src={p.licensePhoto} alt="License" className="max-w-full h-auto rounded border" style={{ maxHeight: '200px' }} />
            </div>
          )}
        </div>
        <div>
          <div className="font-semibold mb-2">Car Information</div>
          ...
          {p.carPhoto && (
            <div className="mt-2">
              ...
              <img src={p.carPhoto} alt="Car" className="max-w-full h-auto rounded border" style={{ maxHeight: '200px' }} />
            </div>
          )}
        </div>
      </div>
...
// Render route application submission
function renderRouteApplication(it) {
  const p = it?.payload || {};
  return (
    <div className="mt-4 space-y-3">
      <div className="grid md:grid-cols-2 gap-4">
        <div className="p-3 rounded bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800">
          ...
          {p.targetRoute ? (
            <>
              <div className="text-sm font-medium text-gray-900 dark:text-gray-100">{p.targetRoute.name}</div>
              <div className="text-xs text-gray-600 dark:text-gray-400 mt-1">Fare: ${p.targetRoute.fare}</div>
              <div className="text-xs text-gray-600 dark:text-gray-400">Active Drivers: {p.targetRoute.activeDriverCount}</div>
            </>
          ) : (
            <>
              <div className="text-sm text-gray-600 dark:text-gray-400">Route ID: {p.targetRouteId || 'N/A'}</div>
        ...
        {p.currentRouteId && (
          <div className="p-3 rounded bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800">
            ...
            {p.currentRoute ? (
              <>
                <div className="text-sm font-medium text-gray-900 dark:text-gray-100">{p.currentRoute.name}</div>
                <div className="text-sm text-gray-600 dark:text-gray-400 mt-1">Months Served: {p.monthsServed || 0}</div>
              </>
            ) : (
              <>
      ...
      {p.reason && (
        <div className="p-3 rounded bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700">
          <div className="font-semibold mb-1">Transfer Reason</div>
          <div className="text-sm text-gray-600 dark:text-gray-400">{p.reason}</div>
        </div>
      )}
      {p.monthsServed !== undefined && p.monthsServed < 3 && (
        <div className="p-3 rounded bg-yellow-100 dark:bg-yellow-900/30 border border-yellow-400">
