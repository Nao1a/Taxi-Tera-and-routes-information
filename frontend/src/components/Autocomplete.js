  ...
  return (
    <div ref={wrapperRef} className="relative w-full">
      <input
        ...
        className={`w-full p-4 rounded-2xl bg-gray-800 border border-gray-700 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
      />
      {open && (
        <div className="absolute left-0 right-0 mt-1 bg-gray-900/95 backdrop-blur-md border border-gray-700 rounded-xl shadow-2xl max-h-64 overflow-y-auto z-20 animate-fadeIn">
          {filtered.length === 0 && (
          ...
          {filtered.map((opt, idx) => (
            <button
              ...
              className={`w-full text-left px-4 py-2 text-sm transition-colors ${idx === highlight ? 'bg-blue-600 text-white' : 'text-gray-200 hover:bg-gray-700 hover:text-white'}`}
            >
